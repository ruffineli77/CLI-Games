...
def lockpick_attempt():  # add a chance event of braking the lock.
    lockpick_line = "A locked door... do you want to try and pick it? (Y/N): "
    lp_success_line = "It worked! The door is unlocked."
    lp_failure_line = "Your lockpick broke, want to use another? (Y/N): "
    ...
    while True:
        ...
        if player_choice in ["y", "Y", "yes", "Yes"]:
            ...
            if attempt == 15:
                ...
                print(attempt)
                print(lp_failure_line)
                time.sleep(2)

        elif player_choice in ["n", "N", "no", "No"]:
            print("You put your tools away and step away from the door")
            time.sleep(2)
            return


# door1 = lockpick_attempt()


def break_attempt():  # add a chance event of braking the lock. Add a mercy rule if the player cant get the right random number
    lockpick_line = "A locked door... do you want to try and break it open? (Y/N): "
    br_success_line = "It worked! The door has broken open."
    br_failure_line = "The door didnt break but your back might have, want to try again? (Y/N): "
    ...
    while True:
        ...
        if player_choice in ["y", "Y", "yes", "Yes"]:
            ...
            if attempt == 15:
                ...
                print(attempt)
                print(br_failure_line)
                time.sleep(2)

        elif player_choice in ["n", "N", "no", "No"]:
            print("You stretch your arms and step away from the door")
            time.sleep(2)
            return
...
def magic_attempt():  # add a chance event of braking the lock. Add a mercy rule if the player cant get the right random number
    lockpick_line = "A locked door... do you want to try and unlocking spell? (Y/N): "
    mg_success_line = "The spell worked! The door flies open."
    mg_failure_line = "The door didnt open. Maybe your footing was off, want to try again? (Y/N): "
    ...
    while True:
        ...
        if player_choice in ["y", "Y", "yes", "Yes"]:
            ...
            if attempt == 15:
                ...
                print(attempt)
                print(mg_failure_line)
                time.sleep(2)

        elif player_choice in ["n", "N", "no", "No"]:
            print("You lower your hands and step away from the door")
            time.sleep(2)
            return
